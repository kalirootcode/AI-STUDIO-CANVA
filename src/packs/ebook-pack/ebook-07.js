...
export function render(data) {
    ...
    const d = {
        TITLE: data.TITLE || '*Fin* del Capítulo',
        CTA: data.CTA || 'Si este contenido te ha sido útil, *guarda* este recurso para futuras consultas, compártelo con tus colegas y *sígueme* para acceder a más bibliotecas de E-books gratuitos.',
        MOTIVATION: data.MOTIVATION || 'El conocimiento en *ciberseguridad* es la mejor inversión. Cada comando aprendido te acerca un paso más a dominar el arte del *hacking* ético.',
        HASHTAGS: data.HASHTAGS || '#Hacking #Ciberseguridad #KRCLIDN'
    };
...
<head>
    <meta charset="UTF-8">
    <title>EBOOK-07 (Back Cover)</title>
    <style>
        ${EBOOK_STYLES}
    </style>
</head>
<body>
    <div class="ebook-circuit-bg" style="transform: rotate(180deg) scale(-1, 1); opacity: 0.1;"></div>

    <div class="safe-zone">
        ${getEbookHeader('KR-CLIDN END')}

        <div style="flex:1; display:flex; flex-direction:column; justify-content:center; align-items:center; text-align:center;">
            
            <div style="font-size: 100px; margin-bottom: 40px;">💾</div>
            
            <h1 class="ebook-h1" style="font-size: 80px; margin-bottom: 30px;">
                ${TemplateUtils.renderEditable('TITLE', formatTitle(esc(d.TITLE)), data._overrides)}
            </h1>
            
            <p class="ebook-p" style="max-width: 85%;">
