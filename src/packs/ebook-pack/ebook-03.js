...
export function render(data) {
    ...
    const d = {
        TITLE: data.TITLE || '¿Qué es el *Hacking* Ético?',
        CONTENT: data.CONTENT || 'El *hacking* ético consiste en utilizar las mismas técnicas que un atacante malicioso, pero con autorización legal y el objetivo de fortalecer la seguridad de sistemas, redes y aplicaciones. Es una disciplina clave en la *ciberseguridad* moderna.\\n\\nLos profesionales certificados como *pentesters* simulan ataques reales para descubrir vulnerabilidades antes de que sean explotadas. Esto incluye pruebas de penetración, análisis de *malware*, ingeniería social y auditorías de infraestructura completa.',
        CURRENT_PAGE: data.CURRENT_PAGE || '3',
        TOTAL_PAGES: data.TOTAL_PAGES || '7'
    };

    // Format content with ebook-p if multiple paragraphs
    const paragraphs = d.CONTENT.split('\\n\\n').map(p => `<p class="ebook-p">${formatContent(esc(p))}</p>`).join('');

    return `<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <title>EBOOK-03 (Text Page)</title>
    <style>
        ${EBOOK_STYLES}
    </style>
</head>
