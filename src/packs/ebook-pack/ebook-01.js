...
export function render(data) {
    ...
    const d = {
        TITLE: data.TITLE || 'Hackeo en *3 Pasos*',
        SUBTITLE: data.SUBTITLE || 'La guía definitiva para *principiantes*',
        AUTHOR: data.AUTHOR || 'KR-CLIDN'
    };
...
<body>
    <div class="ebook-circuit-bg"></div>

    <div class="safe-zone">
        ${getEbookHeader()}

        <div class="cover-content">
            <h2 class="ebook-h2" style="color: var(--primary-color); margin-bottom: 12px; font-weight: 500;">
                ${TemplateUtils.renderEditable('SUBTITLE', esc(d.SUBTITLE), data._overrides)}
            </h2>
            <h1 class="ebook-h1" style="font-size: 110px;">
                ${TemplateUtils.renderEditable('TITLE', formatTitle(esc(d.TITLE)), data._overrides)}
            </h1>
            
            <div style="margin-top: 60px; display:inline-block; border-top: 1px solid rgba(255,255,255,0.2); padding-top: 24px;">
                <span class="mono" style="font-size:38px; color:var(--text-muted); letter-spacing:4px;">AUTOR</span><br>
                <span class="ebook-p" style="font-weight:700; color:#fff;">${esc(d.AUTHOR)}</span>
            </div>
        </div>
