...
export function render(data) {
    ...
    const d = {
        TITLE: data.TITLE || '*Estadísticas* Globales',
        SUBTITLE: data.SUBTITLE || 'Análisis de las principales *vulnerabilidades* y tipos de ataques detectados a nivel global, evidenciando las áreas críticas que demandan atención inmediata en la industria de *ciberseguridad*.',
        STATS: data.STATS || [
            { LABEL: 'Phishing', VALUE: 75, COLOR: 'var(--primary-color)' },
            { LABEL: 'Malware', VALUE: 55, COLOR: 'var(--accent-color)' },
            { LABEL: 'DDoS', VALUE: 30, COLOR: 'var(--warning-color)' }
        ],
        CURRENT_PAGE: data.CURRENT_PAGE || '5',
        TOTAL_PAGES: data.TOTAL_PAGES || '7'
    };

    const chartsHTML = d.STATS.map(s => `
        <div style="margin-bottom: 40px;">
            <div style="display:flex; justify-content:space-between; margin-bottom:16px;">
                <span style="font-family: var(--font-title); font-size: 48px; font-weight:700; color:#fff;">${esc(s.LABEL)}</span>
                <span style="font-family: var(--font-mono); font-size: 46px; font-weight:800; color:${s.COLOR};">${s.VALUE}%</span>
            </div>
            <div style="height: 24px; background: rgba(255,255,255,0.05); border-radius: 12px; overflow:hidden;">
                <div style="height:100%; width:${s.VALUE}%; background:${s.COLOR}; border-radius:12px; box-shadow:0 0 30px ${s.COLOR}60;"></div>
            </div>
        </div>
    `).join('');
...
<head>
    <meta charset="UTF-8">
    <title>EBOOK-05 (Charts Page)</title>
    <style>
        ${EBOOK_STYLES}
    </style>
</head>
<body>
    <div class="safe-zone">
        ...
        <p class="ebook-p ebook-p-muted">${TemplateUtils.renderEditable('SUBTITLE', formatContent(esc(d.SUBTITLE)), data._overrides)}</p>
