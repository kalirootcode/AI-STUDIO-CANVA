...
export function render(data) {
    ...
    const d = {
        TITLE: data.TITLE || '*Hoja* de Trucos',
        INTRO: data.INTRO || 'Referencia rápida de los comandos más utilizados en *Nmap* para escaneo de redes y descubrimiento de servicios.',
        COMMANDS: data.COMMANDS || [
            { CMD: 'nmap -sS', DESC: 'Escaneo Stealth SYN' },
            { CMD: 'nmap -O', DESC: 'Detección de Sistema Operativo' },
            { CMD: 'nmap -p-', DESC: 'Escanear todos los puertos' },
            { CMD: 'nmap -A', DESC: 'Escaneo Agresivo Completo' }
        ],
        TIP: data.TIP || 'Combina *-sV* con *-sC* para obtener versiones de servicios y ejecutar scripts de detección automáticamente.',
        CURRENT_PAGE: data.CURRENT_PAGE || '6',
        TOTAL_PAGES: data.TOTAL_PAGES || '7'
    };

    const cmdsHTML = d.COMMANDS.map(c => `
        <div class="terminal-window" style="margin-bottom: 24px; box-shadow: none;">
            <div class="term-header" style="background: rgba(255,255,255,0.02); padding: 12px 20px;">
                <span class="mono" style="font-size: 30px; color:var(--text-muted);"># ${esc(c.DESC)}</span>
            </div>
            <div class="term-body" style="padding: 20px; font-size: 30px;">
                $ ${esc(c.CMD)}
            </div>
        </div>
    `).join('');
...
<head>
    <meta charset="UTF-8">
    <title>EBOOK-06 (Cheat Sheet)</title>
    <style>
        ${EBOOK_STYLES}
    </style>
</head>
<body>
    <div class="safe-zone">
        ...
        <p class="ebook-p ebook-p-muted" style="margin-bottom: 30px;">
            ${TemplateUtils.renderEditable('INTRO', formatContent(esc(d.INTRO)), data._overrides)}
        </p>
        ...
        <div style="background: rgba(255,255,255,0.03); border-left: 3px solid var(--primary-color); padding: 20px 24px; border-radius: 0 12px 12px 0; margin-top: 20px;">
            <span style="font-family:var(--font-title); font-size: 30px; color: var(--primary-color);">💡 PRO TIP</span>
            <p class="ebook-p" style="font-size: 30px; margin-top: 12px; margin-bottom: 0;">
                ${TemplateUtils.renderEditable('TIP', formatContent(esc(d.TIP)), data._overrides)}
            </p>
        </div>
