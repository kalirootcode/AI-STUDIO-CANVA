...
export function render(data) {
    ...
    const d = {
        TITLE: data.TITLE || 'Comando: *htop*',
        DESCRIPTION: data.DESCRIPTION || '*htop* es la versión mejorada del clásico `top`. Proporciona una interfaz visual, interactiva y con colores para monitorizar los recursos de tu *sistema* en tiempo real.\\n\\nA diferencia de `top`, puedes navegar con las flechas, filtrar procesos por nombre, y eliminar tareas directamente con *F9*. Es una herramienta indispensable en cualquier servidor *Linux*.',
        COMMAND: data.COMMAND || 'sudo apt install htop && htop',
        OUTPUT: data.OUTPUT || '[Visualiza barras de CPU, memoria y una\\n lista de procesos que puedes matar con F9]',
        CURRENT_PAGE: data.CURRENT_PAGE || '4',
...
<head>
    <meta charset="UTF-8">
    <title>EBOOK-04 (Terminal Example)</title>
    <style>
        ${EBOOK_STYLES}
    </style>
</head>
<body>
    <div class="safe-zone">
        ...
        <div>${d.DESCRIPTION.split('\\n\\n').map(p => '<p class="ebook-p">' + formatContent(esc(p)) + '</p>').join('')}</div>

        <!-- Giant Terminal -->
        <div class="terminal-window" style="display:flex; flex-direction:column; margin-top:20px; border-color: var(--primary-color); height: fit-content; max-height: 100%;">
            <div class="term-header" style="justify-content: flex-start; border-bottom-color: var(--primary-color);">
                <div class="term-dot red"></div><div class="term-dot yellow"></div><div class="term-dot green"></div>
                <div class="mono" style="margin-left: 12px; font-size: 30px; color: var(--text-muted);">bash/shell</div>
            </div>
            <div class="term-body" style="display:flex; flex-direction:column; gap: 24px;">
                <div style="color: #fff;">$ <span style="color: var(--success-color);">${esc(d.COMMAND)}</span></div>
                <div style="color: var(--text-muted); font-size: 30px; white-space: pre-wrap; opacity:0.8;">${esc(d.OUTPUT)}</div>
            </div>
        </div>
