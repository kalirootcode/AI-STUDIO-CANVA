    ...
    const nodesHTML = d.NODES.map(n => {
        ...
        return `
            <div class="node ${stClass}">
                <div class="node-icon">${(n.ICON).includes(':') ? '<span class="iconify" data-icon="' + (n.ICON) + '"></span>' : '<i class="material-icons">' + (n.ICON) + '</i>'}</div>
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .node-active .node-icon .iconify { color: #2563EB; font-size: 36px; }
        .node-target .node-icon .iconify { color: #ff3366; font-size: 36px; }
        ...
        .desc-box .iconify { color: #2563EB; font-size: 30px; flex-shrink: 0; }
