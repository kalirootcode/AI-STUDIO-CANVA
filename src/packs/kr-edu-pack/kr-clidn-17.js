...
export function render(data) {
    ...
    const checksHTML = d.CHECK_ITEMS.map(c => `
        <div class="check-row ${c.CHECKED ? 'checked' : ''}">
            <div class="check-box"><i class="material-icons">${c.CHECKED ? 'check_box' : 'check_box_outline_blank'}</i></div>
            <span>${c.TEXT}</span>
        </div>`).join('\n');
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .check-box i { font-size: 32px; }
        .check-row.checked .check-box i { color: #4DD9C0; }
        .check-row:not(.checked) .check-box i { color: rgba(255,255,255,0.2); }
        ...
        .note-box i { color: #2563EB; font-size: 30px; }
