/**
 * KR-CLIDN-15: CONCEPTO / DEFINICIÓN
...
export function render(data) {
    const d = {
        ...
        KEY_POINTS: data.KEY_POINTS || [
            { ICON: 'dns', TEXT: 'Punto clave 1' },
            { ICON: 'lan', TEXT: 'Punto clave 2' },
            { ICON: 'router', TEXT: 'Punto clave 3' }
        ],
    ...
    const pointsHTML = d.KEY_POINTS.map(p => `
        <div class="point-item">
            <div class="point-icon"><i class="material-icons">${p.ICON || 'check'}</i></div>
            <span>${p.TEXT}</span>
        </div>`).join('\n');
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* ═══ KEY POINTS ═══ */
        .points { display: flex; flex-direction: column; gap: 14px; margin-bottom: 36px; }
        ...
        .point-icon i { color: #a855f7; font-size: 30px; }
        ...
        .example-box i { color: #2563EB; font-size: 32px; margin-top: 2px; }
        ...
        .footer-accent { width: 60px; height: 3px; background: linear-gradient(90deg, #a855f7, transparent); }
