/**
 * KR-CLIDN-01: COVER / PORTADA
 * Primer slide SIEMPRE. Logo grande centrado, título, subtítulo, tags.
 * Fondo negro puro, sin animaciones.
 */
export function render(data) {
    const d = {
        TITLE: data.TITLE || 'NMAP',
        SUBTITLE: data.SUBTITLE || 'El escáner de redes más poderoso del mundo',
        TAGS: data.TAGS || [
            { ICON: 'radar', TEXT: 'Escaneo de puertos' },
            { ICON: 'security', TEXT: 'Detección de vulnerabilidades' },
            { ICON: 'dns', TEXT: 'Descubrimiento de redes' }
        ],
        SLIDE_NUMBER: data.SLIDE_NUMBER || '01/08'
    };

    const tagsHTML = d.TAGS.map(t => `
        <div class="tag">
            <i class="material-icons">${t.ICON}</i>
            <span>${t.TEXT}</span>
        </div>`).join('\n');

    return `<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;500;700;800&family=Inter:wght@400;500;600;700&display=swap" rel="stylesheet">
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            font-family: 'Inter', sans-serif;
            background: #000000;
            color: #fff;
            width: 1080px;
            height: 1920px;
            overflow: hidden;
            position: relative;
        }

        .slide {
            position: relative; z-index: 1;
            width: 100%; height: 100%;
            padding: 60px;
            display: flex; flex-direction: column;
            justify-content: center; align-items: center;
        }

        /* ═══ LOGO CENTRAL ═══ */
        .logo-container {
            display: flex;
            flex-direction: column;
            align-items: center;
            margin-bottom: 60px;
        }

        .logo-img {
            width: 200px; height: 200px;
            object-fit: contain;
            margin-bottom: 32px;
            filter: drop-shadow(0 0 30px rgba(37, 99, 235, 0.3));
        }

        .brand-text {
            font-family: 'JetBrains Mono', monospace;
            font-size: 42px; font-weight: 700;
            letter-spacing: 10px;
            color: #2563EB;
        }

        .brand-divider {
            width: 120px; height: 2px;
            background: linear-gradient(90deg, transparent, #2563EB, transparent);
            margin: 24px 0;
        }

        /* ═══ TITLE ═══ */
        .title {
            font-family: 'JetBrains Mono', monospace;
            font-size: 120px; font-weight: 800;
            text-align: center;
            background: linear-gradient(135deg, #2563EB, #4DD9C0);
            -webkit-background-clip: text;
            -webkit-text-fill-color: transparent;
            background-clip: text;
            line-height: 1.1;
            margin-bottom: 24px;
        }

        .subtitle {
            font-size: 36px;
            color: #94A3B8;
            text-align: center;
            max-width: 800px;
            line-height: 1.5;
            margin-bottom: 60px;
        }

        /* ═══ TAGS ═══ */
        .tags {
            display: flex;
            gap: 16px;
            flex-wrap: wrap;
            justify-content: center;
        }

        .tag {
            display: flex; align-items: center; gap: 10px;
            background: rgba(37, 99, 235, 0.08);
            border: 1px solid rgba(37, 99, 235, 0.2);
            border-radius: 100px;
            padding: 14px 24px;
        }

        .tag i { color: #2563EB; font-size: 24px; }
        .tag span {
            font-family: 'JetBrains Mono', monospace;
            font-size: 22px; color: #94A3B8;
        }

        /* ═══ FOOTER ═══ */
        .slide-footer {
            position: absolute; bottom: 60px;
            left: 60px; right: 60px;
            display: flex; justify-content: space-between;
            align-items: center;
        }

        .footer-accent { width: 60px; height: 3px; background: linear-gradient(90deg, #2563EB, transparent); }
        .slide-num { font-family: 'JetBrains Mono', monospace; font-size: 32px; color: rgba(255,255,255,0.3); }

        .corner-tl { position: absolute; top: 60px; left: 60px; width: 40px; height: 40px; border-left: 2px solid rgba(37,99,235,0.15); border-top: 2px solid rgba(37,99,235,0.15); }
        .corner-tr { position: absolute; top: 60px; right: 60px; width: 40px; height: 40px; border-right: 2px solid rgba(37,99,235,0.15); border-top: 2px solid rgba(37,99,235,0.15); }
        .corner-bl { position: absolute; bottom: 60px; left: 60px; width: 40px; height: 40px; border-left: 2px solid rgba(37,99,235,0.15); border-bottom: 2px solid rgba(37,99,235,0.15); }
        .corner-br { position: absolute; bottom: 60px; right: 60px; width: 40px; height: 40px; border-right: 2px solid rgba(37,99,235,0.15); border-bottom: 2px solid rgba(37,99,235,0.15); }
    </style>
</head>
<body>
    <div class="slide">
        <div class="corner-tl"></div>
        <div class="corner-tr"></div>

        <div class="logo-container">
            <img class="logo-img" src="data:image/png;base64," alt="KR-CLIDN">
            <div class="brand-text">KR-CLIDN</div>
            <div class="brand-divider"></div>
        </div>

        <div class="title">${d.TITLE}</div>
        <div class="subtitle">${d.SUBTITLE}</div>

        <div class="tags">${tagsHTML}</div>

        <div class="slide-footer">
            <div class="footer-accent"></div>
            <div class="slide-num">${d.SLIDE_NUMBER}</div>
        </div>
        <div class="corner-bl"></div>
        <div class="corner-br"></div>
    </div>
</body>
</html>`;
}
