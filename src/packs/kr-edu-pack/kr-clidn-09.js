/**
 * KR-CLIDN-09: CTA / FOLLOW
 * Último slide SIEMPRE. Logo, recap de logros, mensaje persuasivo, handle.
 * Fondo negro puro, sin animaciones.
 */
export function render(data) {
    const d = {
        TITLE: data.TITLE || '¿Te fue útil?',
        RECAP_ITEMS: data.RECAP_ITEMS || [
            { ICON: 'check_circle', TEXT: 'Dominas los comandos básicos de Linux' },
            { ICON: 'check_circle', TEXT: 'Sabes usar el terminal como un pro' },
            { ICON: 'check_circle', TEXT: 'Estás listo para el siguiente nivel' }
        ],
        CTA_MESSAGE: data.CTA_MESSAGE || 'Síguenos para más contenido como este todos los días',
        HANDLE: data.HANDLE || '@kr.clidn',
        HASHTAGS: data.HASHTAGS || '#KaliLinux #CyberSecurity #HackingÉtico',
        SLIDE_NUMBER: data.SLIDE_NUMBER || '08/08'
    };

    const recapHTML = d.RECAP_ITEMS.map(item => `
        <div class="recap-item">
            <i class="material-icons">${item.ICON}</i>
            <span>${item.TEXT}</span>
        </div>`).join('\\n');

    return `<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=JetBrains+Mono:wght@400;500;700;800&family=Inter:wght@400;500;600;700&display=swap" rel="stylesheet">
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body {
            font-family: 'Inter', sans-serif;
            background: #000000;
            color: #fff;
            width: 1080px;
            height: 1920px;
            overflow: hidden;
            position: relative;
        }

        .slide {
            position: relative; z-index: 1;
            width: 100%; height: 100%;
            padding: 60px;
            display: flex; flex-direction: column;
            justify-content: center; align-items: center;
        }

        /* ═══ LOGO ═══ */
        .logo-img {
            width: 140px; height: 140px;
            object-fit: contain;
            margin-bottom: 24px;
            filter: drop-shadow(0 0 20px rgba(37, 99, 235, 0.3));
        }

        .brand-text {
            font-family: 'JetBrains Mono', monospace;
            font-size: 36px; font-weight: 700;
            letter-spacing: 8px;
            color: #2563EB;
            margin-bottom: 48px;
        }

        /* ═══ TITLE ═══ */
        .title {
            font-family: 'JetBrains Mono', monospace;
            font-size: 72px; font-weight: 800;
            text-align: center;
            color: #fff;
            margin-bottom: 48px;
        }

        /* ═══ RECAP ═══ */
        .recap-section {
            width: 100%; max-width: 800px;
            margin-bottom: 48px;
        }

        .recap-label {
            font-family: 'JetBrains Mono', monospace;
            font-size: 22px; color: rgba(255,255,255,0.3);
            letter-spacing: 3px; margin-bottom: 20px;
            text-align: center;
        }

        .recap-item {
            display: flex; align-items: center; gap: 16px;
            padding: 18px 0;
            border-bottom: 1px solid rgba(37,99,235,0.08);
        }

        .recap-item i {
            color: #4DD9C0; font-size: 32px; flex-shrink: 0;
        }

        .recap-item span {
            font-size: 30px; color: #94A3B8; line-height: 1.4;
        }

        /* ═══ CTA BOX ═══ */
        .cta-box {
            background: rgba(37, 99, 235, 0.06);
            border: 1px solid rgba(37, 99, 235, 0.2);
            border-radius: 20px;
            padding: 40px;
            width: 100%; max-width: 800px;
            text-align: center;
            margin-bottom: 32px;
        }

        .cta-message {
            font-size: 34px; color: #fff;
            line-height: 1.5; margin-bottom: 24px;
        }

        .cta-arrows {
            font-size: 40px; color: #2563EB;
            letter-spacing: 8px;
            margin-bottom: 16px;
        }

        .handle {
            font-family: 'JetBrains Mono', monospace;
            font-size: 52px; font-weight: 800;
            background: linear-gradient(135deg, #2563EB, #4DD9C0);
            -webkit-background-clip: text;
            -webkit-text-fill-color: transparent;
            background-clip: text;
        }

        /* ═══ HASHTAGS ═══ */
        .hashtags {
            font-family: 'JetBrains Mono', monospace;
            font-size: 22px; color: rgba(255,255,255,0.25);
            text-align: center;
            letter-spacing: 2px;
        }

        /* ═══ FOOTER ═══ */
        .slide-footer {
            position: absolute; bottom: 60px;
            left: 60px; right: 60px;
            display: flex; justify-content: space-between;
            align-items: center;
        }

        .footer-accent { width: 60px; height: 3px; background: linear-gradient(90deg, #2563EB, transparent); }
        .slide-num { font-family: 'JetBrains Mono', monospace; font-size: 32px; color: rgba(255,255,255,0.3); }

        .corner-bl { position: absolute; bottom: 60px; left: 60px; width: 40px; height: 40px; border-left: 2px solid rgba(37,99,235,0.15); border-bottom: 2px solid rgba(37,99,235,0.15); }
        .corner-br { position: absolute; bottom: 60px; right: 60px; width: 40px; height: 40px; border-right: 2px solid rgba(37,99,235,0.15); border-bottom: 2px solid rgba(37,99,235,0.15); }
    </style>
</head>
<body>
    <div class="slide">
        <img class="logo-img" src="data:image/png;base64," alt="KR-CLIDN">
        <div class="brand-text">KR-CLIDN</div>

        <div class="title">${d.TITLE}</div>

        <div class="recap-section">
            <div class="recap-label">HOY APRENDISTE</div>
            ${recapHTML}
        </div>

        <div class="cta-box">
            <div class="cta-message">${d.CTA_MESSAGE}</div>
            <div class="cta-arrows">▼ ▼ ▼</div>
            <div class="handle">${d.HANDLE}</div>
        </div>

        <div class="hashtags">${d.HASHTAGS}</div>

        <div class="slide-footer">
            <div class="footer-accent"></div>
            <div class="slide-num">${d.SLIDE_NUMBER}</div>
        </div>
        <div class="corner-bl"></div>
        <div class="corner-br"></div>
    </div>
</body>
</html>`;
}
