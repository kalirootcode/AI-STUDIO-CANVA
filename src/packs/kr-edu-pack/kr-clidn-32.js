                ...
                <div style="display:flex; flex-direction:column; align-items:center; gap:14px;">
                    <div style="width:140px; height:140px; border-radius:50%; background:conic-gradient(${s.COLOR} ${s.VALUE}%, #1a1a1a ${s.VALUE}%); display:flex; align-items:center; justify-content:center;">
                        <div style="width:100px; height:100px; border-radius:50%; background:#0a0a0a; display:flex; align-items:center; justify-content:center;">
                            <span style="font-family:var(--font-mono); font-size:28px; font-weight:800; color:${s.COLOR};">${s.VALUE}%</span>
                        </div>
                    </div>
                    <span style="font-family:var(--font-mono); font-size:20px; color:#94a3b8; letter-spacing:1px;">${esc(s.LABEL)}</span>
                </div>
            `).join('')}
        </div>`;
    } else {
            ...
            ${d.STATS.map(s => `
                <div>
                    <div style="display:flex; justify-content:space-between; margin-bottom:8px;">
                        <span style="font-size:22px; font-weight:600; color:#e2e8f0;">${esc(s.LABEL)}</span>
                        <span style="font-family:var(--font-mono); font-size:22px; font-weight:700; color:${s.COLOR};">${s.VALUE}%</span>
                    </div>
...
<body>
    ...
    <div class="safe-zone">
        ...
        <div style="display:flex; align-items:center; opacity:0.5;">
            ...
            <span class="mono" style="letter-spacing:2px; font-size:14px;">CYBER-CANVAS // STATS</span>
        </div>
    </div>
