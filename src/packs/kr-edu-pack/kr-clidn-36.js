    ...
    const eventsHTML = d.EVENTS.map((e, i) => `
        <div style="display:flex; gap:16px; align-items:flex-start; position:relative;">
            ...
            <div style="display:flex; flex-direction:column; align-items:center; min-width:60px;">
                <div class="mono" style="font-size:38px; color:var(--primary-color); font-weight:700;">${esc(e.TIME)}</div>
            ...
            <!-- Event dot -->
            <div style="width:40px; height:40px; background:rgba(0,0,0,0.8); border:2px solid var(--primary-color); border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:38px; flex-shrink:0; margin-top:2px; box-shadow:0 0 15px color-mix(in srgb, var(--primary-color) 25%, transparent);">${e.ICON || '⚡'}</div>
            <!-- Content -->
            <div style="flex:1; padding-bottom:16px;">
                <div style="font-size:41px; font-weight:700; color:#fff; margin-bottom:4px;">${esc(e.EVENT)}</div>
                <div style="font-size:38px; color:#aaa; line-height:1.4;">${esc(e.DETAIL)}</div>
            </div>
        </div>`).join('\n');
...
<body>
    ...
    <div class="safe-zone">
        ...
        <div class="mono" style="font-size:38px; color:var(--accent-color); letter-spacing:3px; margin-bottom:8px;">🔥 TIMELINE</div>
        ...
        <div class="glass-panel" style="display:flex; gap:12px; align-items:center; border-color:color-mix(in srgb, var(--accent-color) 15%, transparent);">
            ...
            <span style="font-size:38px; color:#ffffff; line-height:1.4;">${TemplateUtils.renderEditable('IMPACT', `${esc(d.IMPACT)}`, data._overrides)}</span>
        </div>
    </div>
