 ...
 * Combines Material Design (priority) and Lucide Icons
 */

// ICONS MATERIAL (prioridad)
...
// Combinado
const ALL_ICONS = { ...LUCIDE_ICONS, ...MATERIAL_ICONS };

class IconsClass {
    constructor() {
        this.icons = ALL_ICONS;
     ...
     * getIcon - Unified API
     * Supports both:
     * 1. Icons.getIcon(name, size, color) -> Material style
     * 2. Icons.getIcon(name, options = {color, size, strokeWidth, className}) -> Lucide style
     */
    getIcon(name, arg2, arg3) {
        let options = {};
        
        if (typeof arg2 === 'object' && arg2 !== null) {
            options = arg2;
        } else {
            options.size = arg2 !== undefined ? arg2 : 24;
            options.color = arg3 !== undefined && arg3 !== null ? arg3 : 'currentColor';
        }
        ...
        if (this.icons[name]) {
            if (MATERIAL_ICONS[name]) {
                ...
                return `<svg xmlns="http://www.w3.org/2000/svg" height="${size}" viewBox="0 0 24 24" width="${size}" ${fill} class="kr-icon kr-icon-${name} ${className}">
                    ${this.icons[name]}
                </svg>`;
            } else {
                // Render as Lucide Icon
                return this.renderSVG(this.icons[name], { color, size, strokeWidth, className });
            }
        }

        // Fallback
        console.warn(`Icon "${name}" not found. Using placeholder.`);
        return MATERIAL_ICONS['info'] 
            ? `<svg xmlns="http://www.w3.org/2000/svg" height="${size}" viewBox="0 0 24 24" width="${size}" fill="${color}" class="kr-icon kr-icon-info ${className}">${MATERIAL_ICONS['info']}</svg>`
            : this.getPlaceholderIcon({ color, size, className });
    }

    renderSVG(pathData, options) {
        const { color = 'currentColor', size = 24, strokeWidth = 2, className = '' } = options;
        return `<svg 
...
export { Icons as default, Icons };

// Support Node and Browser formats seamlessly
