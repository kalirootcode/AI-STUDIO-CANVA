/**
 * IconService - Generador de Iconos SVG Profesionales
 * Usa Lucide Icons (1,400+ iconos modernos)
 * https://lucide.dev/icons
 */

class IconService {
    constructor() {
        // CDN de Lucide Icons (versión estática)
        this.cdnBase = 'https://cdn.jsdelivr.net/npm/lucide-static@latest/icons';
        this.cache = new Map();
    }

    /**
     * Genera un icono SVG inline con colores personalizados
     * @param {string} name - Nombre del icono (ej: 'shield', 'lock', 'terminal')
     * @param {Object} options - Opciones de personalización
     * @param {string} options.color - Color del icono (hex)
     * @param {number} options.size - Tamaño en px
     * @param {number} options.strokeWidth - Grosor del trazo
    ...
    getIcon(name, options = {}) {
        ...
        // Iconos SVG embebidos (los más usados para evitar requests)
        const embeddedIcons = this.getEmbeddedIcons();

        if (embeddedIcons[name]) {
            return this.renderSVG(embeddedIcons[name], { color, size, strokeWidth, className });
        }

        // Si no está embebido, usar placeholder
        console.warn(`Icon "${name}" not found in embedded icons. Using placeholder.`);
        return this.getPlaceholderIcon({ color, size, className });
    }

    /**
     * Renderiza un SVG con los parámetros dados
     */
    renderSVG(pathData, options) {
        const { color, size, strokeWidth, className } = options;
    ...
    /**
     * Icono placeholder cuando no se encuentra el solicitado
     */
    getPlaceholderIcon(options) {
        const pathData = '<circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/>';
        return this.renderSVG(pathData, options);
    }

    /**
     * Iconos embebidos (50+ iconos más usados)
     * Path data extraído de Lucide Icons
     */
    getEmbeddedIcons() {
        return {
            // ═══ SEGURIDAD ═══
            ...
            'antenna': '<path d="M2 12 7 2"/><path d="m7 12 5-10"/><path d="m12 12 5-10"/><path d="m17 12 5-10"/><path d="M4.5 17H4a2 2 0 0 1-2-2v-2a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v2a2 2 0 0 1-2 2h-.5"/><path d="M9 22v-6h6v6"/><path d="M9 16v-4h6v4"/>',
            ...
            'stop': '<rect width="18" height="18" x="3" y="3" rx="2" ry="2"/>',
